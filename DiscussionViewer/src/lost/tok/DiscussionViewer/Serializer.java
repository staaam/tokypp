...
import lost.tok.DiscussionViewer.emf.impl.OpinionImpl;
import lost.tok.DiscussionViewer.emf.impl.RootImpl;
...
public class Serializer implements ModelSerializer{
	...
	public ModelInstance readModel(IFile dest, ClassLoader cl, IProgressMonitor monitor) throws Exception{
		...
		resourceSet.getPackageRegistry().put(lost.tok.DiscussionViewer.emf.impl.DiscussionViewerPackageImpl.eINSTANCE.getNsURI(),lost.tok.DiscussionViewer.emf.impl.DiscussionViewerPackageImpl.eINSTANCE);
		

	    resourceSet.getResourceFactoryRegistry().getExtensionToFactoryMap().put("dis", 
		...
		lost.tok.DiscussionViewer.Root root = null;
		if(contents.size() > 0){
			lost.tok.DiscussionViewer.emf.Root eroot = (lost.tok.DiscussionViewer.emf.Root)contents.get(0);
			...
			for(Object m : mementos){
				lost.tok.DiscussionViewer.emf.Memento memento = (lost.tok.DiscussionViewer.emf.Memento)m;
				try{
			...
			root = (lost.tok.DiscussionViewer.Root)inst.getRoot();
			DiscussionViewer rr = null;
			if(eroot.getRealRoot() != null)
				rr = (DiscussionViewer)factory.getPart(eroot.getRealRoot());
			if(rr == null)
			   rr = new DiscussionViewer();
		    EList list = ((RootImpl)contents.get(0)).getOpinion();
						
			//root.addChild((LogicElement) ((OpinionImpl) list.get(1)).eInternalContainer());
		    root.setRealRoot(rr);
			...
			root = new lost.tok.DiscussionViewer.Root();
			
		    root.setRealRoot(new DiscussionViewer());
	...
	public String getType(){
		return "DiscussionViewer to XMI";
	}
	...
	public void serializeToXMI(ModelObject obj, IFile dest) {
		...
		lost.tok.DiscussionViewer.Root root = (lost.tok.DiscussionViewer.Root)ModelRepository.getInstance().getInstanceRepository().getInstance(obj.getModelInstanceID()).getRoot();
		...
		DiscussionViewer rr = root.getRealRoot();
