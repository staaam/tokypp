...
public class Quote extends AbstractDicObject  implements Adapter, EMFModelObject, org.gems.designer.model.EMFModelElement{

	private lost.tok.DiscussionViewer.emf.Quote model_;
	private Notifier target_;
	
	public Quote(){		
	}
	
	public Quote(lost.tok.DiscussionViewer.emf.Quote model){
		model_ = model;
	...
	public lost.tok.DiscussionViewer.emf.Quote getModel(){
		if(model_ == null){
			model_ = lost.tok.DiscussionViewer.emf.impl.DiscussionViewerFactoryImpl.eINSTANCE.createQuote();
			super.setName(model_.getName());
			model_.eAdapters().add(this);
		}
		return model_;
	...
	public lost.tok.DiscussionViewer.emf.Quote getExtendedModel(){
		return getModel();
	}
	
	public lost.tok.DiscussionViewer.emf.Quote getEMFObject(){
		return getModel();
	}
    ...
    public static final IPropertyDescriptor[] QUOTE_ATTRIBUTE_DESCRIPTORS =
    {
    };
    public static final Object[] QUOTE_DEFAULT_ATTRIBUTE_VALUES = {
    ...
    protected void getPropertyDescriptors(Vector desc){
    	desc.addAll(java.util.Arrays.asList(QUOTE_ATTRIBUTE_DESCRIPTORS));
   ...
   public void notifyChanged(Notification notification) {
	...
	public List<Opinion> getManyQuotes(){
		return (List<Opinion>)ModelUtilities.getConnectedSources(this,Opinion.class,ContainmentConnectionType.INSTANCE);
	}
...
public void connectInput(Wire w) {
	...
	if(type != null){
		...
		if(type.getName().equals(ContainmentConnectionType.NAME)){
					...
					getModel().getManyQuotes().add(((EMFModelObject)w.getSource()).getEMFObject());
...
public void disconnectInput(Wire w) {
	...
	if(type != null){
		...
		if(type.getName().equals(ContainmentConnectionType.NAME)){
					...
					getModel().getManyQuotes().remove(((EMFModelObject)w.getSource()).getEMFObject());
...
public void disconnectOutput(Wire w, boolean modifymodel) {
	...
	public ConnectionType getConnectionTypeForRole(String role){
	    ...
	    if("manyQuotes".equalsIgnoreCase(role)){
...
public void accept(Visitor visitor) {
   if(visitor instanceof DiscussionViewerVisitor)
   	((DiscussionViewerVisitor)visitor).visitQuote(this);
   else
